...
#### Korean Language
When Korean text is detected, **Nanum Gothic** (`나눔고딕`) is prepended to all font stacks as the primary typeface. Korean text also renders at a **+2px size bump** relative to the base Latin size at every typographic scale step (e.g. if body is 16px, Korean body is 18px) to compensate for the denser glyph structure and improve legibility.

- Display stack (Korean): `"Nanum Gothic", Tiempos Headline, Garamond, "Times New Roman", serif`
- Body / UI stack (Korean): `"Nanum Gothic", Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif`
- Code stack: unchanged (`JetBrains Mono` — Korean identifiers rare in code context)
