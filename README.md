...
*   **Kermany Retinal OCT Dataset (Kaggle)**
    *   **설명:** 약 84,000장의 고해상도 B-Scan 이미지(정상군, CNV, DME, Drusen)로 이루어진 전 세계에서 가장 대중적인 OCT 데이터셋입니다. 픽셀 노이즈 분포가 일정하여 고성능 덴스 비전 아키텍처나 SSL 백본 모델 학습용 기본 데이터로 필수적입니다.
    *   **데이터셋 링크:** [Kaggle - Retinal OCT Images (Optical Coherence Tomography)](https://www.kaggle.com/datasets/paultimothymooney/kermany2018)
*   **AROI (Annotated Retinal OCT Images) Database**
    *   **설명:** 황반변성(AMD) 환자들의 2D OCT 볼륨 데이터 1,136장을 미세 망막 레이어(Layers) 및 유체 축적 영역(Fluid Accumulation) 단위로 전문의가 정밀 주석(Annotation)을 달아놓은 데이터셋입니다. 치주선(Periodontal Line) 이나 접합상피 세그멘테이션 모델 구조 검증에 최적입니다.
    *   **데이터셋 가이드 링크:** [AROI Database for Joint Retinal Layer and Fluid Segmentation](https://www.tandfonline.com/doi/full/10.1080/00051144.2021.1973298)
*   **MedSegBench (종합 의료 영상 세그멘테이션 벤치마크)**
    *   **설명:** *Nature Scientific Data (2024)*에 등재된 데이터셋으로, OCT를 포함한 다양한 모달리티의 의료 영상 60,000장 이상을 128/256/512 규격으로 표준화하여 정형화된 Train/Val/Test Split과 함께 제공합니다. 전처리 기법(Bicubic Interpolation 등) 모델 벤치마크에 유용합니다.
    *   **벤치마크 참조 링크:** [MedSegBench: A Comprehensive Benchmark in Diverse Data Modalities](https://pmc.ncbi.nlm.nih.gov/articles/PMC11589128/)
...
-  ① 하드웨어 및 데이터 획득 기반 방식 (Compounding)
    *   **프레임 에버리징 (B-scan Frame Averaging):** 동일한 위치를 고속으로 여러 번(예: 20~50회) 촬영한 뒤 평균을 내는 방식입니다. 스페클 노이즈는 무작위적(Stochastic) 특성을 가지므로, 프레임을 평균 내면 노이즈는 상쇄되고 구조적 신호만 남습니다. **현재 임상 장비에서 가장 신뢰하는 기준(Ground Truth)**이지만, 촬영 시간이 늘어나 환자의 움직임 Artifact가 생길 수 있습니다.
    *   **공간/주파수 컴파운딩 (Spatial/Frequency Compounding):** 빛의 입사 각도를 미세하게 다르게 하거나(공간), 서로 다른 파장 대역의 빛을 분할 조사하여(주파수) 얻은 영상을 결합합니다. 각기 다른 스페클 패턴을 오버랩시켜 상쇄하는 원리입니다.

-  ② 공간 도메인 필터링 (Spatial Domain Filtering)
    *   **국소 적응형 필터 (Local Adaptive Filters):** 이미지의 특정 윈도우(예: 3x3, 5x5) 내에서 평균과 분산을 계산하여 노이즈를 줄입니다.
        *   *Lee, Kuan, Frost 필터:* 픽셀 주변의 국소 통계치를 기반으로, 변화가 적은 평탄한 구역은 강하게 부드럽게(Smoothing) 만들고, 경계면(Edge)은 필터링을 약하게 하여 구조를 보존합니다. 단, 연산은 빠르지만 경계면이 다소 흐려지는 블러링(Blurring) 현상이 있습니다.
    *   **이방성 확산 필터 (Anisotropic Diffusion / SRAD):** 픽셀 간 밝기 기울기(Gradient)를 분석하여 경계면과 수평인 방향으로만 확산(Smoothing)을 진행합니다. 특히 **SRAD(Speckle Reducing Anisotropic Diffusion)**는 스페클의 곱셈성 특성을 미분 방정식에 반영하여, 치은선이나 망막 레이어의 경계면을 매우 날카롭게 유지하면서 내부 노이즈만 지워내는 탁월한 능력을 보여줍니다.

-  ③ 비국소 및 변환 도메인 필터링 (Advanced DSP)
    *   **비국소 평균 필터 (Non-Local Means, NLM):** 특정 윈도우 안만 보는 것이 아니라, 이미지 전체(또는 넓은 영역)에서 **기하학적으로 유사한 패턴을 가진 패치(Patch)**들을 찾아내어 이들의 가중 평균을 구합니다. OCT 특유의 반복적인 레이어 구조를 보존하는 데 매우 효과적이지만, 연산량이 극도로 많다는 단점이 있습니다.
    *   **웨이블릿 임계화 (Wavelet Thresholding):** 영상을 주파수 영역(Wavelet Domain)으로 변환하면 고주파 영역에 스페클 노이즈 계수들이 집중됩니다. 특정 임계값(Threshold) 이하의 계수들을 제거하거나 줄인 뒤 역변환(Inverse Transform)하는 방식입니다.
...
- ① 지도 학습 기반 방식 (Supervised Denoising)
    *   **쌍을 이룬 데이터 학습 (Paired Data Learning):**
        ...
        *   *모델 아키텍처:* **DnCNN(Denoising CNN)**이나 **U-Net** 구조를 주로 사용하며, 최근에는 전역적 문맥을 파악하는 트랜스포머 기반의 **Restormer**나 **SwinIR** 등이 활용됩니다.
        *   *손실 함수:* 단순 L1, L2 Loss만 사용하면 평균적인 값으로 수렴하여 이미지가 흐려지기 때문에, 사람의 시각적 인지 경계를 보존하는 **VGG 기반 Perceptual Loss(지각적 손실)**나 **SSIM(Structural Similarity) Loss**를 결합하여 치주 연조직의 미세한 텍스처를 살려냅니다.

-  ② 자가 학습 및 비지도 학습 기반 방식 (Self-Supervised / Unsupervised)
임상 환경에서 완벽히 깨끗한 Ground Truth(GT) 이미지를 얻기 어려울 때 사용하는 돌파구입니다.
    *   **Noise2Noise (N2N):** 완전히 깨끗한 타깃 영상이 없더라도, **동일한 부위를 촬영한 노이즈 낀 영상 2장**만 있으면 학습이 가능합니다. 두 영상의 스페클 노이즈는 통계적으로 독립적이라는 점을 이용하여, AI가 서로의 노이즈를 예측하는 과정에서 노이즈의 평균값(0)을 찾아내어 신호만 남기게 만듭니다.
    *   **Noise2Void (N2V) / Blind Spot Network:** 단 한 장의 노이즈 영상으로도 학습이 가능합니다. 특정 픽셀의 값을 주변 픽셀들을 통해 예측(Blind-spot)하도록 모델을 제어하면, 무작위로 발생하는 스페클 노이즈는 예측할 수 없고 연속적인 조직 구조 신호만 예측할 수 있게 되어 자연스럽게 노이즈가 걸러집니다.
    *   **CycleGAN 기반 Domain Translation:** 노이즈가 있는 이미지 그룹(Domain A)과 노이즈가 없는 타깃 그룹(Domain B)이 서로 1:1로 매칭되지 않더라도, 두 도메인 간의 스타일 전환 메커니즘을 통해 노이즈 특성만 지워내는 비지도 방식입니다.

- ③ 생성형 모델 기반 방식 (Generative Priors)
    *   **Diffusion Plug-and-Play (PnP) Priors:** 최근 가장 각광받는 기법으로, 깨끗한 의료 영상의 분포를 먼저 학습한 확산 모델(Diffusion Model)을 노이즈 제거 과정의 역방향(Reverse Process) 조건으로 주입합니다. 단순 매핑보다 누락된 레이어 구조를 복원하는 능력이 매우 뛰어나며, 초해상도(Super-Resolution) 작업과 동시에 수행하기 적합합니다.

#### 스페클 노이즈 제거 데이터셋
학습 방식별 데이터 요구사항 정리

지도학습 (Supervised)   → PKU37, D1, Sub2Full vis-OCT  (noisy-clean 쌍 필수)
Noise2Noise            → 동일 위치 noisy 2장 필요 → RETOUCH 볼륨에서 추출 가능
Noise2Void / Self2Self → 단일 noisy 이미지면 충분 → 보유 데이터 전부 활용 가능
Diffusion PnP          → 깨끗한 이미지 분포 사전학습 → Kermany ~84,000장 활용 적합

권장 방향: Clean GT를 확보하기 어려운 치주 OCT 도메인에서는 Noise2Void 또는 Self2Self 방식이 현실적이며, 보유 중인 AROI·Kermany 데이터를 바로 사용할 수 있습니다. 정량 벤치마크가 필요하다면 PKU37을 우선 다운로드하는 것을 권장합니다.       

Sources:
- PKU37 - Aliyun Tianchi (https://tianchi.aliyun.com/dataset/133217)
- D1 / Deep Learning Denoise OCT ONH - Scientific Reports (https://www.nature.com/articles/s41598-019-51062-7)
- Sub2Full - arXiv (https://arxiv.org/abs/2401.10128)
- RETOUCH Challenge (https://retouch.grand-challenge.org/)
- ODTiD - ICPSR (https://doi.org/10.3886/E137701V3)
- OCT Denoising Systematic Review - PMC (https://pmc.ncbi.nlm.nih.gov/articles/PMC11050869/)
- Sub2Full GitHub (https://github.com/PittOCT/Sub2Full-OCT-Denoising)
...
### kaggle - Retinal OCT Imgaes
https://www.kaggle.com/datasets/paultimothymooney/kermany2018?resource=download

### AROI Dataset
데이터 접근 권한 요청 메일 전송 - 응답 대기 중
Download: https://ipg.fer.hr/ipg/resources/oct_image_database
Code: https://github.com/mmelinscak/OCT-images-segmentation

### MedSegBench
Download: https://zenodo.org/records/13359660
Code: https://github.com/zekikus/MedSegBench
