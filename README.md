...
*   **AROI (Annotated Retinal OCT Images) Database**
    *   **설명:** 황반변성(AMD) 환자들의 3D OCT 볼륨 데이터 1,136장을 미세 망막 레이어(Layers) 및 유체 축적 영역(Fluid Accumulation) 단위로 전문의가 정밀 주석(Annotation)을 달아놓은 데이터셋입니다. 치주선(Periodontal Line) 이나 접합상피 세그멘테이션 모델 구조 검증에 최적입니다.
