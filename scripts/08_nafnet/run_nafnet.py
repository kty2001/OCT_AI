"""
8단계: NAFNet 백본으로 6-fold CV 지도학습.

DnCNN / U-Net 대비 NAFNet 아키텍처 효과 측정.
초기화: from scratch
Loss: L1 + 0.1*(1-SSIM)
출력: clean 이미지 (global residual 포함, 별도 clamp 불필요)
...
결과:
  ...
  results/08_nafnet/images/fold_{k}/sample_*.png

VRAM 가이드 (RTX A4000 16GB, 128x128 패치 기준):
  width=16 (~4M params)  : batch=32 권장
  width=32 (~17M params) : batch=16 권장
  width=64 (~67M params) : batch=8  권장
"""
...
RESULTS_DIR = ROOT / "results" / "08_nafnet"
...
DNCNN_REF = {"psnr": 28.17, "ssim": 0.6732, "cnr": 1.167}
...
def _print_vram(device: torch.device):
    props = torch.cuda.get_device_properties(device)
    total = props.total_memory / 1e9
    print(f"VRAM: {total:.1f} GB ({props.name})")
...
def main():
    ...
    for fold in range(1, K_FOLDS + 1):
        ...
        # --start-fold: 이미 완료된 fold는 기존 결과를 읽어 집계에 포함
    ...
    for key, label in [("psnr", "PSNR"), ("ssim", "SSIM"), ("cnr", "CNR")]:
        ...
        print(f"{label:4s}: {val:.4f} +- {std:.4f}  "
              f"(SRAD {b:.3f} {flag} / U-Net {u:.4f} / DnCNN {d:.4f})")
    print("=================================================")


def parse_args():
    p = argparse.ArgumentParser(description="NAFNet 6-fold CV 지도학습")
    # 모델
    p.add_argument("--width",      type=int,   default=32,
                   help="기본 채널 수 (16=~4M / 32=~17M / 64=~67M params)")
    p.add_argument("--enc-blks",   type=str,   default="1,1,1,28",
                   help="인코더 각 스테이지 NAFBlock 수 (쉼표 구분)")
